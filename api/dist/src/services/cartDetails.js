...
const removeProductFromCart = (productId) => {
    server_1.client.query('SELECT * FROM public."cartDetails"  WHERE  "productId"=$1 ', [productId], (error, results) => {
        if (error) {
            //record error in the error file
            logger_1.default.error(error.detail);
            throw error;
        ...
        if (results.rowCount == 1) {
            if (Number(results.rows[0].quantity) > 1) {
                server_1.client.query('Update public."cartDetails" SET "quantity"="quantity"-1 WHERE  "productId"=$1 ', [productId], (error, results) => {
                    if (error) {
                        //record error in the error file
                        logger_1.default.error(error.detail);
                        throw error;
...
const getCartDetails = (cartId) => __awaiter(void 0, void 0, void 0, function* () {
    const response = yield server_1.client.query('SELECT  cartdetails.quantity as amount, product.id, product.category,product.description,product.image,product.price, product.title, cartdetails.price as total FROM public."cartDetails" cartdetails, public."product" product  WHERE "cartId"=$1 And cartdetails."productId"=product."id"', [cartId]);
    console.log(response.rows);
    return response.rows;
});
